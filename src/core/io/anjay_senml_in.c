...
static int get_i64_from_number(int64_t *out_value,
                               const anjay_json_like_number_t *number) {
    if (number->type == ANJAY_JSON_LIKE_VALUE_UINT) {
        ...
        *out_value = (int64_t) number->value.f64;
        ...
        LOG(WARNING, _("expected numeric type, got something else instead"));
        return -1;
    }
    return 0;
}
...
static int get_i64(senml_in_t *in, int64_t *out_value) {
    ...
    return get_i64_from_number(out_value, &value);
}
...
static int read_some_cached_bytes(senml_in_t *in,
    ...
    if (!can_return_value(in) || in->entry->type != bytes_type) {
        return -1;
    }
...
static int senml_get_some_bytes(anjay_input_ctx_t *ctx,
...
static int
senml_get_string(anjay_input_ctx_t *ctx, char *out_buf, size_t buf_size) {
    assert(buf_size);
    senml_in_t *in = (senml_in_t *) ctx;
...
static int senml_get_integer(anjay_input_ctx_t *ctx, int64_t *out_value) {
    senml_in_t *in = (senml_in_t *) ctx;
    if (!can_return_value(in)) {
        return -1;
    }
    if (!is_type_numeric(in->entry->type)
            || get_i64_from_number(out_value, &in->entry->value.number)) {
...
static int senml_get_uint(anjay_input_ctx_t *ctx, uint64_t *out_value) {
    senml_in_t *in = (senml_in_t *) ctx;

    if (!is_type_numeric(in->entry->type)) {
        return ANJAY_ERR_BAD_REQUEST;
    }
    if (in->entry->value.number.type == ANJAY_JSON_LIKE_VALUE_UINT) {
        *out_value = (uint64_t) in->entry->value.number.value.u64;
    } else if (in->entry->value.number.type
               == ANJAY_JSON_LIKE_VALUE_NEGATIVE_INT) {
        LOG(WARNING, _("expected positive number, got NEGATIVE INT"));
        return ANJAY_ERR_BAD_REQUEST;
    } else if (in->entry->value.number.type == ANJAY_JSON_LIKE_VALUE_FLOAT
               && avs_double_convertible_to_uint64(
                          in->entry->value.number.value.f32)) {
        *out_value = (uint64_t) in->entry->value.number.value.f32;
    } else if (in->entry->value.number.type == ANJAY_JSON_LIKE_VALUE_DOUBLE
               && avs_double_convertible_to_uint64(
                          in->entry->value.number.value.f64)) {
        *out_value = (uint64_t) in->entry->value.number.value.f64;
    } else {
        LOG(WARNING,
            _("expected positive integer, got negative or non-integer "
...
static int senml_get_double(anjay_input_ctx_t *ctx, double *out_value) {
    senml_in_t *in = (senml_in_t *) ctx;
    if (!can_return_value(in)) {
        return -1;
    }
    ...
    if (in->entry->value.number.type == ANJAY_JSON_LIKE_VALUE_FLOAT) {
        *out_value = in->entry->value.number.value.f32;
    } else if (in->entry->value.number.type == ANJAY_JSON_LIKE_VALUE_DOUBLE) {
        *out_value = in->entry->value.number.value.f64;
    } else if (in->entry->value.number.type == ANJAY_JSON_LIKE_VALUE_UINT) {
        *out_value = (double) in->entry->value.number.value.u64;
    } else if (in->entry->value.number.type
               == ANJAY_JSON_LIKE_VALUE_NEGATIVE_INT) {
        *out_value = (double) in->entry->value.number.value.i64;
    } else {
        return ANJAY_ERR_BAD_REQUEST;
    }
    in->value_read = true;
...
static int senml_get_bool(anjay_input_ctx_t *ctx, bool *out_value) {
    senml_in_t *in = (senml_in_t *) ctx;
...
static int senml_get_objlnk(anjay_input_ctx_t *ctx,
                            anjay_oid_t *out_oid,
                            anjay_iid_t *out_iid) {
    senml_in_t *in = (senml_in_t *) ctx;
    ...
    if (anjay_get_string(ctx, objlnk, sizeof(objlnk))) {
        return ANJAY_ERR_BAD_REQUEST;
    }
...
static int senml_get_path(anjay_input_ctx_t *ctx,
...
static int senml_next_entry(anjay_input_ctx_t *ctx) {
    senml_in_t *in = (senml_in_t *) ctx;
    if (in->path.ids[0] == ANJAY_ID_INVALID) {
        return 0;
    }
...
static int senml_close(anjay_input_ctx_t *ctx) {
    senml_in_t *in = (senml_in_t *) ctx;
...
static int
input_senml_create(anjay_input_ctx_t **out,
                   anjay_json_like_decoder_t *ctx,
...
int _anjay_input_senml_cbor_create(anjay_input_ctx_t **out,
                                   avs_stream_t **stream_ptr,
                                   const anjay_uri_path_t *request_uri) {
    anjay_json_like_decoder_t *cbor_ctx = _anjay_cbor_decoder_new(*stream_ptr);
    if (!cbor_ctx) {
        return -1;
    }
...
int _anjay_input_json_create(anjay_input_ctx_t **out,
                             avs_stream_t **stream_ptr,
                             const anjay_uri_path_t *request_uri) {
